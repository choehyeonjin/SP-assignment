...
// -----------------------------------------------------------------
// Title        : System Programming proxy Assignment #1-3
// Description  : A concurrent proxy server program
//				  that converts the input URL into SHA1
//                and creates cache directory, file and miss/hit/runtime log
// =================================================================
...
// Purpose      : Handling command from user,
//				       creating cache file based on SHA1 hashed url,
//                    writing hit/miss and run time log
// =================================================================
...
int main() {
	...
	char home[256];
	getHomeDir(home); // get home dir path
	...
	while (1) {
		...
		scanf("%s", cmd); // read input command

		if (strcmp(cmd, "connect") == 0) { // connect to child process
			numOfChild++;
			...
			if (pid == 0) { // child
				...
				while (1) {
					...
					scanf("%s", input_url); // read input url

					time_t t;
					time(&t); // get current time when input url
					struct tm* lt = localtime(&t); 

					if (strcmp(input_url, "bye") == 0) { // exit child process if input "bye"
						time_t childEndTime;
						time(&childEndTime); // save chile end time
						int childRunTime = childEndTime - childStartTime; // calc chile run time
						fprintf(fp, "[Terminated] run time: %d sec. #request hit : %d, miss : %d\n",
							childRunTime, hit, miss); // write child terminate log
			...
			time(&serverEndTime);
			int serverRunTime = serverEndTime - startTime;
			fprintf(fp, "**SERVER** [Terminated] run time: %d sec. #sub process: %d\n",
				serverRunTime, numOfChild);
