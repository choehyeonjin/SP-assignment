...
// -----------------------------------------------------------------
// Title        : System Programming proxy Assignment #3-1
// Description  : A proxy server that caches web content and logs
//                client requests using named semaphore to protect
//                logfile.txt from concurrent access.
// =================================================================
...
// -----------------------------------------------------------------
// Purpose      : Setup signal handlers for SIGINT and SIGCHLD
// =================================================================
void signalHandler() {
		...
		if (pid == 0) { // child process
			...
			if (hitFlag) {
				...
				if (isURL) {
					...
					sleep(1); // simulate concurrent access
					printf("*PID# %d is in the critical zone.\n", getpid());
					...
					if (fp) {
						fprintf(fp, "[HIT]%s/%s-[%04d/%02d/%02d, %02d:%02d:%02d]\n[HIT] %s\n", cache_dirname, cache_filename,
							...
							lt->tm_hour, lt->tm_min, lt->tm_sec, url);
					}
					fclose(fp);
					printf("*PID# %d exited the critical zone.\n", getpid());
